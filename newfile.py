...
# --- CLIENT DATABASE (Panel ke andar hi clients add honge) ---
CLIENT_DB = {} # Format: {'client_username': 'password'}
...
# --- TERI ORIGINAL SIREN LISTS (Ab List format mein bilkul sahi) ---
SIREN_LIST_1 = [
    "𝗔𝗡𝗧𝗘𝗥 𝗠𝗔𝗡𝗧𝗘𝗥 𝗦𝗛𝗘𝗧𝗔𝗡𝗜 𝗞𝗛𝗢𝗣𝗗𝗔 < {target}> 𝗧𝗘𝗥𝗜 𝗠𝗔𝗔 𝗞𝗔 𝗕𝗛𝗢𝗦𝗗𝗔 🪼⋆｡𖦹°🫧⋆.ೃ࿔*:･ --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------𝗔𝗡𝗧𝗘𝗥 𝗠𝗔𝗡𝗧𝗘𝗥 𝗦𝗛𝗘𝗧𝗔𝗡𝗜 𝗞𝗛𝗢𝗣𝗗𝗔  < {target}> 𝗧𝗘𝗥𝗜 𝗠𝗔𝗔 𝗞𝗔 𝗕𝗛𝗢𝗦𝗗𝗔 🪼⋆｡𖦹°🫧⋆.ೃ",
    "𝗔𝗡𝗧𝗘𝗥 𝗠𝗔𝗡𝗧𝗘𝗥 𝗦𝗛𝗘𝗧𝗔𝗡𝗜 𝗞𝗛𝗢𝗣𝗗𝗔 < {target}> 𝗧𝗘𝗥𝗜 𝗠𝗔𝗔 𝗞𝗔 𝗕𝗛𝗢𝗦𝗗𝗔 🪼⋆｡𖦹°🫧⋆.ೃ࿔*:･ --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------𝗔𝗡𝗧𝗘𝗥 𝗠𝗔𝗡𝗧𝗘𝗥 𝗦𝗛𝗘𝗧𝗔𝗡𝗜 𝗞𝗛𝗢𝗣𝗗𝗔  < {target}> 𝗧𝗘𝗥𝗜 𝗠𝗔𝗔 𝗞𝗔 𝗕𝗛𝗢𝗦𝗗𝗔 🪼⋆｡𖦹°🫧⋆.ೃ࿔*:･"
]

# FIXED SIREN_LIST_2 (Agar isme extra braces/commas the toh maine theek kar diya)
SIREN_LIST_2 = [
...
def run_spam_worker(session_id, initial_target, custom_texts_list, template_list, target_scope, target_gc_input, custom_delay, module_key, uname):
    ...
    while active_spam_threads.get(module_key, False):
        ...
        try:
            cl = Client()
            cl.login_by_sessionid(session_id)
            log_event(uname, f"Authenticated with Jitter Pacing.", "success")
            ...
            while active_spam_threads.get(module_key, False):
                ...
                try:
                    if not target_threads:
                        log_event(uname, f"No GCs detected. Retrying...", "warning")
                        time.sleep(3)
                        continue
                    ...
                    for thread_id in target_threads:
                        ...
                        try:
                            ...
                            consecutive_errors = 0
                            log_event(uname, f"SENT ➔ Target: {current_target} | GC: {str(thread_id)[:8]}...", "success")
                            ...
                            if "block" in err_msg.lower() or "limit" in err_msg.lower():
                                log_event(uname, f"SPAM DETECTED! Quarantine 60s...", "error")
                                node_quarantine_status[uname] = True
                                def release_quarantine(u):
                                    time.sleep(60)
                                    node_quarantine_status[u] = False
                                    log_event(u, f"Quarantine finished!", "success")
                                q_thread = threading.Thread(target=release_quarantine, args=(uname,))
                                q_thread.daemon = True
                                q_thread.start()
                                break
                            else:
                                log_event(uname, f"Error: {err_msg[:35]}", "error")
                            if consecutive_errors > 4:
                                log_event(uname, f"Cooling node for 15s...", "warning")
                                time.sleep(15)
                                consecutive_errors = 0
                    time.sleep(2)
                except:
            ...
            log_event(uname, f"Session Sync Error. Re-connecting...", "error")
            time.sleep(10)
...
# ============================================================
# ORIGINAL DASHBOARD TEMPLATE + CREATE CLIENT BUTTON (Only Admin)
# ============================================================
DASHBOARD_HTML = """
<!DOCTYPE html>
<html>
...
def index():
    ...
    if request.method == "POST":
        ...
        # Baki actions (Client bhi use kar sakta hai, par data usika alag dikhega)
            ...
            try:
                ...
                # Save with owner tag (Admin ya Client)
                conn = sqlite3.connect('panel_users.db')
                cursor = conn.cursor()
            ...
            if new_tgt:
                # Sirf apne nodes ka target update ho
                conn = sqlite3.connect('panel_users.db')
                cursor = conn.cursor()
                ...
                conn.close()
                message = f"Target switched to: '{new_tgt}' for {username}'s nodes!"
                log_event(username, f"Live target switched: {new_tgt}", "success")
            ...
            if not selected_nodes:
                ...
                for uname in selected_nodes:
                    ...
                    if row:
                        ...
                        t.daemon = True
                        t.start()
                        log_event(username, f"Node @{uname} deployed on '{target_name}'", "info")
            ...
            log_event(username, "Emergency stop executed.", "warning")
